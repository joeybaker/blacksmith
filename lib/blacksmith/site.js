...
var events = require('events')
    ...
    , feed = new Feed({
      title: 'Joey Baker'
      , 'site_url': 'https://byjoeybaker.com'
    , });
...
Site.prototype.load = function (callback) {
  var dir = path.normalize(this.dir), self = this;
  ...
  common.loadSite(dir, function (err, results) {
      ...
      if (results[key] && self[key]) {
        Object.keys(results[key]).forEach(function (item) {
          self[key][item] = utile.mixin(
            self[key][item] || {}, results[key][item]
          );
        });
      }
      else {
    ...
    callback(null, {
      dir:        self.dir, html:       self.html, pages:      self.pages, layout:     self.layout, layouts:    self.layouts, partials:   self.partials, references: self.references
    });
  });
};
    ...
    function renderPages (content, next) {
      ...
      Object.keys(self.pages).forEach(
        function renderPage (name) {
          ...
          //
          var page = new Page({
            name:       name, dir:        self.dir, layout:     self.pages[name].layout || self.layout, options:    self.pages[name], html:       self.html, content:    content, references: self.references, partials:   self.partials
          });
  ...
  fs.readdir(dir, function (err, files) {
    ...
    async.parallel([
      ...
      function renderMarkdowns (done) {
        async.forEachLimit(
          markdown, 10, function renderMarkdown (file, next) {
            var fullpath = path.join(dir, file), ext = path.extname(file), name = path.basename(file, ext);
            ...
            fs.stat(fullpath, function (err, stats) {
              ...
              content.render({
                source: fullpath, marked: self.marked, files: supporting, dir: dir
                ...
                rendered[name] = {
                  _content: results, _date: results.metadata.date
                    ? new Date(results.metadata.date)
                    : new Date(stats.ctime)
                };

                common.addDetails(rendered[name]._content, {
                  dir: path.join(self.dir, 'content'), fullpath: fullpath, files: supporting, stats: stats
                });

                // create an item in the RSS feed
                feed.item({
                  title: results.metadata['page-details'].title, description: results.truncated.html, url: results.metadata['page-details'].href, author: results.metadata.author, date: rendered[name]._date
                })
      ...
      function renderDirs (done) {
        async.forEachLimit(
          dirs, 10, function renderDir (file, next) {
...
Site.prototype.writeFiles = function (rendered, callback) {
  var publicDir = path.join(this.dir, 'public'), written = {}, self = this;
  ...
  async.series([
    ...
    function writeAll (done) {
      async.forEachSeries(
        self.targetFiles(rendered), function write (target, next) {
          //
...
Site.prototype.targetFiles = function (rendered, pathTo) {
  var contentDir = path.join(this.dir, 'content'), publicDir = path.join(this.dir, 'public'), self = this;
  ...
  return Object.keys(rendered).reduce(function (all, key) {
    ...
    // hierarchy.
    //
    var fullpath = [publicDir].concat(pathTo), childKeys;
    ...
    if (!rendered[key]._content) {
      ...
      //
      all = all.concat([{
        fullpath: path.join.apply(path, fullpath.concat([key])), contents: 'directory'
      }]);
    }
      ...
      //
      all = all.concat({
        fullpath: path.join.apply(path, fullpath.concat([key + '.html'])), contents: rendered[key]._content.html
      });
      ...
      if (rendered[key]._content.files) {
        all = all.concat(rendered[key]._content.files.map(function (file) {
          return {
            fullpath: path.join.apply(path, fullpath.concat([path.basename(file)])), source: path.join(contentDir, rendered[key].dir || '', file)
          };
        }));
      }
    }
